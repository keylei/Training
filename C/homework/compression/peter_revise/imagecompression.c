...
void exportByteDataToFile( char outputName[],
                           unsigned char *pMappingData,//rime 导出数据为什么用map
                           int arrayLength )
{
...
void compressImage( float *pImageHeight,
                    unsigned char *pMappingData,//rime 一会data, 一会image,到底有啥区别
                    int picLenArr[],
                    int imageCount )
{
    //rime +1 知道调用底层库, 值得鼓励
    char writeName[FILENAME_MAX];   //rime 文件名称就是文件名称, 啥叫writeName, 动词意味太浓
    int picOffset = 0;                                  // 图片在内存地址中的偏移
    int curArrLen = 0;                                  // 待压缩数组长度
    int curPicCnt = 0;                                  // 待压缩图片数量
    int picLen = 0;                                     // 待压缩数组长度
    int temp[4] = {1,0,0,0};                            // 临时表头数组   //rime 临时数组干嘛用的, 要解释解释啊,难道非要我去查找引用才能知道
...
//rime number这种注释谁能明白啥意思
void getNumberFromeString( char string[],
                           int number[] )//rime 这种两个段参数, 就不需要一行一个了
{
    int count=0;
    int index=0;
    int temp=0;
...
void exportFloatDataToFile( char outputName[],
                            float *pDiffData,//rime 导出数据为什么用diff
                            int arrayLength )//rime 有些地方用array, 有写地方用arr
{
    FILE *file = fopen( outputName, "w");               // 打开文件，不存在则创建
    if ( file == NULL )
    {
...
void decompressImage( float *pImageHeight,
                      float *pDecompressData, //参数为什么和compressImage不对仗, 这里是pDecompressData, 上面就是pMappingData了
                      int picLenArr[],
                      int imageCount )
{
