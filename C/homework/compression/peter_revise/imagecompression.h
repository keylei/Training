...
#define COMPRESS_LENGTH 4096                // 图像长度大于4096直接压缩,小于则多个图像一起压缩 //rime 为什么一会length, 一会len
#define MAX_DECOMPRESS_LEN 10000            // 待压缩数组最大长度

//rime 导出的路径应该在main函数中设置
#define EXPORT_PATH "./data/"               // 导出压缩文件的路径  //rime 这里想说明的是压缩数据的生成目录路径, 这样命名看不出来是压缩数据的意思
#define DIFF_PATH "./diff/"                 // 导出差值文件的路径  //rime 目录路径, 应该明确表示目录
 ...
 * @return                      N/A
 */
void getNumberFromeString( char string[], //rime 单词还能够拼错
                           int number[] );
...
int mappingImageToArray( float* pImageHeight,
                         int headerDataArr[], //rime 声明的时候是数组, 为什么定义的时候又变成指针了呢
...
void importDataFromFile( char fileName[],
                         ...
                         int* pCacheData,//rime 为什么会有这种命名
                         int picLenArr[] );

/**
 * @brief compressImage         压缩图像信息并导出到文件
 *                              if(图像长度<4096)                        //rime 注释中都用白话文解释了, 不要用伪代码, 哪里有用伪代码去注释代码的
 *                                  {将图片存入待压缩数组}
 *                              else (图像长度>4096)
 *                                  {直接压缩图片}
...
void compressImage( float *pImageHeight,
                    unsigned char *pMappingData,
                    int picLenArr[], //rime 大量用了img, 就不要出现pic了
                    int imageCount );

/**
 * @brief decompressImage       解压缩图像信息并计算与原图片高度的差值
 *                              1.打开文件夹(判断是否存在)  //rime 这种注释不就比上面的正常多了
 *                              2.打开文件,并将文件中的值存入内存
 *                              3.根据读取的文件信息,将图像高度信息还原
 *                              4.将还原的高度信息与原数据比较差值
 *                              5.打印每张图片的高度值的差值  //rime 要求是解压不需要直接计算diff数据的, 不满足要求
 * @param pPictureData          所有图片的原始高度信息  //rime 哪里有这个变量啊
 * @param pDiffData             存放解压后与原图片高度差值的信息
 * @param perPicLenArr          存放每张图片长度的数组 //rime 直接就lenArr就可以了,这个命名太累赘了
 * @param imageCount            图片数量
 * @return                      N/A
 */
