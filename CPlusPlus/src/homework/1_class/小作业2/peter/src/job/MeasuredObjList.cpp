...
void MeasuredObjList::pushHead(MeasuredObj* pNewMeasuredObj)
{
    ...
    pNewMeasuredObj->setNextMeasuredObj(pOldHead);

    //将原来的首个元素的前一个元素指针指向新元素
    if( nullptr != pOldHead)
        pOldHead->setPreMeasureObj(pNewMeasuredObj);

    //将新元素的前一个元素指针置为空,表示不存在
    pNewMeasuredObj->setPreMeasureObj(nullptr);
}
...
void MeasuredObjList::pushTail(MeasuredObj *pNewMeasuredObj)
{
    ...
    if( nullptr != pOldTail )
    {
        ...
        pOldTail->setNextMeasuredObj( pNewMeasuredObj );
    }
    else
    {
    ...
    pNewMeasuredObj->setPreMeasureObj(pOldTail);

    //将新元素的后一个元素指针置为空,表示不存在
    pNewMeasuredObj->setNextMeasuredObj(nullptr);
...
void MeasuredObjList::pullHead()
{
    ...
    if( nullptr != this->m_pHeadMeasuredObj )
        this->m_pHeadMeasuredObj->setPreMeasureObj(nullptr);
}
...
void MeasuredObjList::pullTail()
{
    ...
    if(nullptr != pNewTail)
        pNewTail->setNextMeasuredObj(nullptr);
}
