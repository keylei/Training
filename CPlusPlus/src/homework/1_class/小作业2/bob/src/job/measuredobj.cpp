...
Job::MeasuredObj::MeasuredObj(std::string name, ShapeType shape)
{
    try
    {
        ...
        if(shape == 0)   //如果形状类型为0,则生成为圆形框
        {
            this->m_pShape = new Circle(NumRandom::randomDouble(maxPos,minPos),
                                 NumRandom::randomDouble(maxPos,minPos),
                                 NumRandom::randomDouble(maxRadius,minRadius));
        }
        else             //如果形状类型为1,则生成为矩形框
        {
            this->m_pShape = new Rectangle(NumRandom::randomDouble(maxPos,minPos),
                                 NumRandom::randomDouble(maxPos,minPos),
