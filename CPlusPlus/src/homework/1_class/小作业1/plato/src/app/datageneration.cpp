...
void DataGeneration::generateImgData(const int& imgDataCnt, unsigned char *pImgData)
{
    for (int i = 0; i < imgDataCnt; ++i)
    {
//        pImgData[i] = NumRandom::generateRandomNum(MIN_GRAY_VALUE,MAX_GRAY_VALUE);
        pImgData[i] = 1;
//        cout << (int)pImgData[i] << endl;
    }
}
